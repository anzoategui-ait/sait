...
if (isset($_POST['actividad_id']) || isset($_POST['usuario_id']) || isset($_POST['fecha_inicio']) || isset($_POST['fecha_final'])) {
...
if ($actividad_id == "" || $fecha_inicio == "" || $fecha_fin == "" || $usuario_id == "") {
    ?>
    <!DOCTYPE html>
    ...
    if ($usuario_id == 0) { //Todos los operadores
        if ($actividad_id == 0) {
            $rs_actividades = $ins_reporte->obtener_actividad_operador_controlador("Actividades", $usuario_id, $actividad_id, $fecha_inicio, $fecha_fin);
        } else {
            //Actividad especifica
            $rs_actividades = $ins_reporte->obtener_actividad_operador_controlador("ActividadesEspecifica", $usuario_id, $actividad_id, $fecha_inicio, $fecha_fin);
        
        }
    } else { //Operador en especifico
        if ($actividad_id == 0) {
            //Todas las actividades
            $rs_actividades = $ins_reporte->obtener_actividad_operador_controlador("ActividadesOperadorEspecifico", $usuario_id, $actividad_id, $fecha_inicio, $fecha_fin);
        
        } else {
            //Actividad especifica
            $rs_actividades = $ins_reporte->obtener_actividad_operador_controlador("ActividadeyOperadoresEspecificos", $usuario_id, $actividad_id, $fecha_inicio, $fecha_fin);
    ...
    $pdf->Cell(0, 10, utf8_decode(strtoupper("Actividades por operador desde el " . $fecha_inicio . " hasta el " . $fecha_fin)), 0, 0, 'L');
    $pdf->Ln(15);
    ...
    $pdf->Cell(50, 8, utf8_decode('OPERADOR'), 1, 0, 'C', false);
    ...
    if ($rs_actividades->rowCount() > 0) {
        ...
        foreach ($rs_actividades as $rows) {
            ...
            $nombre_actividad = substr($nombre_actividad, 0, 50) . "_";
            $pdf->Cell(50, 6, utf8_decode($rows['operador_nombre'] . ', ' . $rows['operador_apellido']), 1, 0, 'L');
