...
if (isset($_POST['solicitud_id_comprobante'])) {
    $solicitud_id = $_POST['solicitud_id_comprobante'];
} else {
    $solicitud_id = "";
}
$peticionAjax = true;
...
if ($solicitud_id == "") {
    ...
    if($detalle_solicitud->rowCount()>0){
        ...
        $solicitante = $detalle_solicitud['usuario_nombre'] . ', ' . $detalle_solicitud['usuario_apellido'] . ' (' . $detalle_solicitud['usuario_dni'] . ')';
    ...
    $pdf->Cell(50, 10, utf8_decode('TELEFONO'), 1, 0, 'C', true);
    $pdf->Cell(50, 10, utf8_decode('EMAIL'), 1, 0, 'C', true);
    $pdf->Ln(10);
