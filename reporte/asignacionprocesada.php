...
if ($asignacion_id == "") {
    ...
    if ($detalle_solicitante->rowCount() > 0) {
        ...
        $solicitante = $detalle_solicitante['usuario_nombre'] . ', ' . $detalle_solicitante['usuario_apellido'];
    }
    ...
    $pdf->Cell(0, 10, utf8_decode(strtoupper("Asignacion # " . $ins_reporte->descifrar($asignacion_id))), 0, 0, 'R');
    $pdf->Ln(15);
    ...
    $pdf->SetFont('Arial', 'B', 10);
    $pdf->Cell(182, 8, utf8_decode('ACTIVIDAD ASIGNADA'), 1, 0, 'L', false);
    $pdf->Ln(8);
    $pdf->SetFont('Arial', '', 8);
    ...
    if ($pasos->rowCount() > 0) {
        ...
        $pdf->SetFont('Arial', 'B', 10);
        $pdf->Cell(182, 8, utf8_decode('DETALLES'), 1, 0, 'L', false);
        $pdf->Ln(8);
        $pdf->SetFont('Arial', '', 8);
        foreach ($pasos as $rows) {
            
            $pdf->MultiCell(0, 6, utf8_decode("PASO " . $cont . ": " . $rows['paso_nombre']), 'LRBT', 'L', false);
            $pdf->MultiCell(0, 6, utf8_decode("OBSERVACION: " . $rows['procesar_observacion']), 'LRB', 'L', false);
            $cont++;
        }
    }
    $cont = 0;
   ...
   $pdf->Ln(5);
    ...
    if ($imagenes->rowCount() > 0) {
        ...
        foreach ($imagenes as $imagen) {
            $pdf->Cell(90, 60, $pdf->Image("../imagenes/" . $imagen['imagen_nombre'], $pdf->GetX(), $pdf->GetY(), 90), 1, 0, 'L', false);
            if($salto == 2){
              $pdf->Ln(70);  
              $salto=0;
            } 
