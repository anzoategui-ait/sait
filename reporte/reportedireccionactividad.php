...
$ins_gabinete = new direccionControlador();
...
if ($actividad_id == "" || $fecha_inicio == "" || $fecha_fin == "" || $gabinete_id == "") {
    ...
    if ($gabinete_id == 0) { //Todos los operadores
        if ($actividad_id == 0) {
            $rs_actividades = $ins_reporte->obtener_actividad_operador_controlador("ActividadesDir", $gabinete_id, $actividad_id, $fecha_inicio, $fecha_fin);
        } else {
            //Actividad especifica
            $rs_actividades = $ins_reporte->obtener_actividad_operador_controlador("ActividadesEspecificaDir", $gabinete_id, $actividad_id, $fecha_inicio, $fecha_fin);
        
        }
    } else { //Operador en especifico
        if ($actividad_id == 0) {
            //Todas las actividades
            $rs_actividades = $ins_reporte->obtener_actividad_operador_controlador("ActividadesDirEspecifico", $gabinete_id, $actividad_id, $fecha_inicio, $fecha_fin);
        
        } else {
            //Actividad especifica
            $rs_actividades = $ins_reporte->obtener_actividad_operador_controlador("ActividadyDirEspecificos", $gabinete_id, $actividad_id, $fecha_inicio, $fecha_fin);
    ...
    $pdf->Cell(0, 10, utf8_decode(strtoupper("Actividades por Direccion desde el " . $fecha_inicio . " hasta el " . $fecha_fin)), 0, 0, 'L');
    $pdf->Ln(15);
    ...
    $pdf->Cell(50, 8, utf8_decode('DIRECCION'), 1, 0, 'C', false);
    ...
    if ($rs_actividades->rowCount() > 0) {
        ...
        foreach ($rs_actividades as $rows) {
            ...
            $pdf->Cell(50, 6, utf8_decode($rows['direccion_nombre']), 1, 0, 'L');
